...
@endif
@if(!empty($trainings))
<h3 style="font-size: 14px; font-weight: 500; color: #585858; line-height: 20px;">{{ __('trainingcalendar::trainingcalendar.registrations') }}</h3>
    <div style="background: #FAF8F5; padding: 16px 11px; border-radius: 8px; margin-top: 16px">
        <table style="width: 100%;">
            <thead>
                <tr>
                <th style="font-size: 14px; padding: 0px 20px; font-weight: 500; color: #585858; line-height: 20px;">{{ $emailFor == 'tutor' ? (setting('_lernen.student_display_name') ?? __('general.student')) : (setting('_lernen.tutor_display_name') ?? __('general.tutor')) }}</th>
                <th style="font-size: 14px; padding: 0px 20px; font-weight: 500; color: #585858; line-height: 20px;">{{ __('trainingcalendar::trainingcalendar.training') }}</th>
                <th style="font-size: 14px; padding: 0px 20px; font-weight: 500; color: #585858; line-height: 20px;">{{ __('trainingcalendar::trainingcalendar.event_datetime') }}</th>
                @if(isPaidSystem())
                    <th style="font-size: 14px; padding: 0px 20px; font-weight: 500; color: #585858; line-height: 20px;">{{ __('courses::courses.price') }}</th>
                @endif
                </tr>
            ...
            <tbody>
                @foreach($trainings as $training)
                    <tr>
                        @php
                            $img  = $emailFor == 'student' ? $training['tutorImg'] : $training['studentImg'];
                            $name = $emailFor == 'student' ? $training['tutorName'] : $training['studentName'];
                        @endphp
                        <td style="display: flex; align-items:center; gap: 10px; padding: 10px 20px;">
                            @if (!empty($img) && Storage::disk(getStorageDisk())->exists($img))
                                <img style="width: 36px; height: 36px; border-radius: 50%;"  src="{{ resizedImage($img, 40, 40) }}" alt="{{ $name }}">
                        ...
                        <td style="font-size: 14px; padding: 10px 20px; font-weight: 400; color: #585858; line-height: 20px;">{!! $training['trainingTitle'] !!} </td>
                        <td style="font-size: 14px; padding: 10px 20px; font-weight: 400; color: #585858; line-height: 20px;"><span style="display: block;">{!! $training['eventDatetime'] !!}</span></td>
                        @if(isPaidSystem())
                            <td style="font-size: 14px; padding: 10px 20px; font-weight: 400; color: #585858; line-height: 20px;"><span style="display: block;">{!! formatAmount($training['trainingPrice']) !!}</span></td>
                        @endif
                    </tr>
                @endforeach
            </tbody>
